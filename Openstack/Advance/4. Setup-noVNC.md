...
|-|-|
| novncproxy_base_url=http://127.0.0.1:6080/vnc_auto.html| Là một địa chỉ cho VNC proxy console. .Địa chỉ IP mà client sẽ kết nối đến . noVC Client sẽ dùng để chỉ này để kết nối tới noVNC Instance. |
|vnc_enabled=True| bật các chức năng liên quan VNC|
| vnc_keymap=en-us|  keymap cho VNC|
|server_listen=127.0.0.1| địa chỉ IP mà các instance sẽ lắng nghe các request VNC trên node này. VNC server sẽ lắng nghe trên trên IP này  |
|server_proxyclient_address=127.0.0.1| địa chỉ IP proxy client  của VNC proxy server |.
