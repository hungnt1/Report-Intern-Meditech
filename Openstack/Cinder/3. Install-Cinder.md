...
```bash
echo "export OS_VOLUME_API_VERSION=2" >> ~/admin_openrc
source ~/admin_openrc
...
openstack endpoint create --region RegionOne   volumev3 admin http://controller:8776/v3/%\(project_id\)s yum install -y openstack-cinder
